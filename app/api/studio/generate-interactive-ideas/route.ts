...
import { generateInteractiveIdeasWithGemini } from "../../../../lib/studio/gemini-shared";
...
function sanitize(body: Record<string, unknown>) {
  ...
  return {
    ...
    interactive_focus: str(body.interactive_focus),
    ...
    objective: str(body.objective) || "comments",
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rl = await rateLimit(uid, "interactive", 20, 60_000);
    if (!rl.allowed) return rateLimitResponse(rl.resetAt - Date.now());

    const body = await req.json().catch(() => ({}));
    const params = sanitize(body as Record<string, unknown>);

    const result = await generateInteractiveIdeasWithGemini(params);
    return NextResponse.json(result);
  } catch (err) {
    return handleApiError(err, "Interactive ideas generation failed.");
  }
}
